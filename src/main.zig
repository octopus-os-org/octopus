...
pub fn main() void {
    ...
    regs.GPIOA.MODER.modify(.{ .MODER9 = 0b10, .MODER10 = 0b10});
    regs.GPIOA.AFRH.modify(.{.AFRH9 = 0b0111, .AFRH10 = 0b0111});

        // Set pin 8 
    regs.GPIOA.MODER.modify(.{ .MODER8 = 0b01});
    // regs.GPIOD.BSRR.modify(.{ .BS8 = 1 });


    regs.USART1.BRR.modify(.{ .DIV_Fraction = 1, .DIV_Mantissa=52});
    regs.USART1.CR1.modify(.{ .UE = 1, .TE=1, .RE = 1, .M=0});
    regs.USART1.DR.write_raw(':');

    // read rcc status
    var cr = regs.RCC.CR.read_raw();
    _ = cr;

    var cfgr = regs.RCC.CFGR.read_raw();
    _ = cfgr;
    ...
    //    if ( regs.USART1.SR.read().RXNE == 1) {
    //     regs.USART1.SR.modify(.{.RXNE=0});
    //     var rd = (regs.USART1.DR.read().DR);
    //     regs.USART1.DR.write_raw(rd&0xff);
    //    }
        regs.USART1.DR.write_raw(65);
    }
}
